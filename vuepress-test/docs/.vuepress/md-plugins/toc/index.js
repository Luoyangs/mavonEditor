...
var defaults = {
  includeLevel: [ 1, 2 ],
  containerClass: 'table-of-contents jsjj',
  slugify: slugify,
  markerPattern: /^\[\[toc\]\]/im,
  listType: 'shopee-anchor',
...
module.exports = function(md, o) {
  ...
  md.renderer.rules.toc_body = function(tokens, index) {
    if (options.forceFullToc) {
      ...
      console.log('toc', renderChildsTokens(0, gstate.tokens)[1])
  ...
  function renderChildsTokens(pos, tokens) {
    ...
    while(i < size) {
      ...
      buffer = `<shopee-anchor-item key="${link}" href="${link}" title="${options.format(content, md, link)}">`;
      i++;
    }
    buffer += buffer === '' ? '' : '</shopee-anchor-item>';
    headings.push(buffer);
    return [i, "<"+ options.listType +">"+ headings.join('') +"</"+ options.listType +">"];
  }
